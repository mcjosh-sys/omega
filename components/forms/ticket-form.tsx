...
import { Contact, Tag, User } from "@/lib/generated/prisma";
import {
  getSubAccountTeamMembers,
  saveActivityAndNotify,
  searchContacts,
  upsertTicket,
} from "@/lib/queries";
import { cn } from "@/lib/utils";
import { useModal } from "@/providers/modal-provider";
import { TicketFormSchema, TicketWithTags } from "@/types";
...
const TicketForm = ({ getNewTicket, laneId, subaccountId }: Props) => {
  ...
  const form = useForm<z.infer<typeof TicketFormSchema>>({
    ...
    defaultValues: {
      ...
      value: String(Number(defaultData.ticket?.value) || 0),
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof TicketFormSchema>) => {
    ...
    try {
      ...
      await saveActivityAndNotify({
        agencyId: undefined,
        desc: `Updated a ticket | ${response?.name}`,
        subaccountId,
      });
                  ...
                  <CommandInput
                    ...
                    onChangeCapture={async (value) => {
                      ...
                      saveTimerRef.current = setTimeout(async () => {
                        const response = await searchContacts(target.value);
                        setContactList(response);
                        setSearch("");
                      }, 1000);
                    }}
