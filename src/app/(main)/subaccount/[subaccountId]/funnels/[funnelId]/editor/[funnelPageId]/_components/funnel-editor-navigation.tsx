...
import { DeviceTypes } from '@/types/editor'
...
const FunnelEditorNavigation: React.FC<Props> = ({
    ...
    const handleOnBlurTitleChange: FocusEventHandler<HTMLInputElement> = async (
        ...
        if (event.target.value) {
            const response = execute(upsertFunnelPage, subaccountId,
                {
            ...
            toast.promise(response, {
                loading: 'Loading...',
                success: () => {
                    return `Saved Funnel Page title`;
                },
                error: 'Unable to save funnel page title',
            });
            router.refresh()
        } else {
            toast.error('Oppse!', {
        ...
        <TooltipProvider>
            <nav
                className={clsx(
                    ...
                    { 'h-0 p-0 hidden': state.editor.previewMode }
                )}
            >
