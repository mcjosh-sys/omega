...
const SubAccountPage = async ({ params: { subaccountId }, searchParams: { code } }: Props) => {
    let currency = 'USD'
    let sessions
    ...
    if (subaccountDetails.connectAccountId) {
        ...
        closingRate = +((totalClosedSessions.length / checkoutSessions.data.length) * 100).toFixed()
    }
                        ...
                        <Card className="flex-1 relative">
                            <CardHeader>
                                ...
                                <CardTitle className="text-4xl">
                                    {`${currency} ${net ? formatter.format(net).replace('$', '') : '0.00'}`}
                                </CardTitle>
