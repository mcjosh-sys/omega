...
import { execute, formatter } from '@/lib/utils'
...
const Page = async ({ params: { agencyId }, searchParams: { code } }: Props) => {
  let currency = 'USD'
  let sessions
  ...
  if (agencyDetails.connectAccountId) {
    ...
    // console.log({ sessions: new Date(1712650937*1000)})
    ...
    closingRate = +((totalClosedSessions.length / checkoutSessions.data.length)*100).toFixed() || 0
  }
        ...
        <div className="flex gap-4 flex-col xl:!flex-row">
          <Card className="flex-1 relative">
            <CardHeader>
              ...
              <CardTitle className="text-4xl">
                {`${currency} ${net ? net.toFixed(2) : '0.00'}`}
              </CardTitle>
          ...
          <Card className="flex-1 relative">
            <CardHeader>
              ...
              <CardTitle className="text-4xl">
                {`${currency} ${potentialIncome ? formatter.format(potentialIncome).replace('$', '') : '0.00'}`}
              </CardTitle>
              <small className="text-xs text-muted-foreground">
                For the year {currentYear}
              </small>
